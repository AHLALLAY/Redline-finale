    ...
    public function loginStaff(array $credentials)
    {
        try {
            ...
            $staffWithDetails = User::where('users.id', $staff->id)
                ...
                ->select('users.id','users.name','users.email','users.cin',
                'users.role','users.birth_date','users.phone','users.last_diploma',
                'users.obtained_at','users.is_suspended', 'users.is_deleted', 'c.level', 'c.group', 's.nom as subject_name')
                ->first();
